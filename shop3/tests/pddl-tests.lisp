...
(fiveam:def-test test-precond-rewriting (:suite pddl-fluents-tests :fixture mock-fluent-domain)
  (iter (for comp-op in +numerical-comparisons+)
    (as sample-expr-1 = `(,comp-op shop-user::(amount jug0) 12))
    (as sample-expr-2 = `(,comp-op 12 shop-user::(capacity jug0)))
    (as sample-expr-3 = `(,comp-op shop-user::(amount jug0) shop-user::(capacity jug0)))

    (as result-1 = `(fluent-check ,comp-op shop-user::(amount jug0) 12))
    (as result-2 = `(fluent-check ,comp-op 12 shop-user::(capacity jug0)))
    (as result-3 = `(fluent-check ,comp-op shop-user::(amount jug0) shop-user::(capacity jug0)))
  ...
  (let ((jug-precond-1 'shop-user::(>= (- (capacity ?jug2) (amount ?jug2)) (amount ?jug1)))
        (jug-precond-2 'shop-user::(and (jug-in-hand) (>= (- (capacity ?jug2) (amount ?jug2)) (amount ?jug1))))
        (jug-precond-3 'shop-user:: (or (and (jug-in-hand) (>= (- (capacity ?jug2) (amount ?jug2)) (amount ?jug1)))
                                        (and (jug-in-robot) (>= (capacity ?jug2) (amount ?jug2)))))
        (jug-precond-4 'shop-user:: (forall (?jug1 ?jug2)
                                            (or (and (jug-in-hand) (>= (- (capacity ?jug2) (amount ?jug2)) (amount ?jug1)))
                                                (and (jug-in-robot) (>= (capacity ?jug2) (amount ?jug2))))))
        (jug-precond-5 'shop-user:: (exists (?jug1 ?jug2)
                                            (or (and (jug-in-hand) (>= (- (capacity ?jug2) (amount ?jug2)) (amount ?jug1)))
                                                (and (jug-in-robot) (>= (capacity ?jug2) (amount ?jug2))))))
        )
    (fiveam:is (equalp
                'shop-user::(shop::fluent-check >= (- (capacity ?jug2) (amount ?jug2)) (amount ?jug1))
                (translate-fluent-precond *domain* jug-precond-1)))
    (fiveam:is (equalp
                'shop-user:: (and (jug-in-hand) (shop::fluent-check >= (- (capacity ?jug2) (amount ?jug2)) (amount ?jug1)))
                (translate-fluent-precond *domain* jug-precond-2)))
    (fiveam:is (equalp
                'shop-user:: (or (and (jug-in-hand) (shop::fluent-check >= (- (capacity ?jug2) (amount ?jug2)) (amount ?jug1)))
                                 (and (jug-in-robot) (shop::fluent-check >= (capacity ?jug2) (amount ?jug2))))
                (translate-fluent-precond *domain* jug-precond-3)))
    (fiveam:is (equalp
                'shop-user:: (forall (?jug1 ?jug2)
                                     (or (and (jug-in-hand) (shop::fluent-check >= (- (capacity ?jug2) (amount ?jug2)) (amount ?jug1)))
                                         (and (jug-in-robot) (shop::fluent-check >= (capacity ?jug2) (amount ?jug2)))))
                (translate-fluent-precond *domain* jug-precond-4)))
    (fiveam:is (equalp
                'shop-user:: (exists (?jug1 ?jug2)
                                     (or (and (jug-in-hand) (shop::fluent-check >= (- (capacity ?jug2) (amount ?jug2)) (amount ?jug1)))
                                         (and (jug-in-robot) (shop::fluent-check >= (capacity ?jug2) (amount ?jug2)))))
...
(fiveam:def-test test-effect-rewriting (:suite pddl-fluents-tests :fixture mock-fluent-domain)
  (iter (for update-op in +fluent-updates+)
    (as sample-expr-1 = `(,update-op shop-user::(amount jug0) 12))
    (as sample-expr-2 = `(,update-op shop-user::(amount jug0) shop-user::(capacity jug0)))

    (as result-1 = `(fluent-update ,update-op shop-user::(amount jug0) 12))
    (as result-2 = `(fluent-update ,update-op shop-user::(amount jug0) shop-user::(capacity jug0)))
  ...
  ;; jug pouring example:
  (fiveam:is (equalp 'shop-user::(and (shop::fluent-update assign (amount ?jug1) 0)
                                      (shop::fluent-update assign (amount ?jug2)
                                                           (+ (amount ?jug1) (amount ?jug2))))
                     (translate-metric-updates *domain* 
                                               'shop-user::(and (assign (amount ?jug1) 0)
                                                                (assign (amount ?jug2)
                                                                        (+ (amount ?jug1) (amount ?jug2)))))))
  (fiveam:is (equalp 'shop-user:: (forall (?jug1 ?jug2) (and (shop::fluent-update assign (amount ?jug1) 0)
                                                             (shop::fluent-update assign (amount ?jug2)
                                                                                  (+ (amount ?jug1) (amount ?jug2)))))
                     (translate-metric-updates *domain* 
                                               'shop-user:: (forall (?jug1 ?jug2)
                                                                    (and (assign (amount ?jug1) 0)
                                                                         (assign (amount ?jug2)
                                                                                 (+ (amount ?jug1) (amount ?jug2))))))))
  (fiveam:is (equalp 'shop-user:: (forall (?jug1 ?jug2) 
                                          (when (and (shop::fluent-check > (amount ?jug2) 0)
                                                     (robotic-jug ?jug2))
                                            (and (shop::fluent-update assign (amount ?jug1) 0)
                                                 (shop::fluent-update assign (amount ?jug2)
                                                                      (+ (amount ?jug1) (amount ?jug2))))))
                     (translate-metric-updates *domain* 
                                               'shop-user:: (forall (?jug1 ?jug2)
                                                                    (when (and (> (amount ?jug2) 0)
                                                                               (robotic-jug ?jug2))
                                                                      (and (assign (amount ?jug1) 0)
                                                                           (assign (amount ?jug2)
                                                                                   (+ (amount ?jug1) (amount ?jug2))))))))))
    ...
    (fiveam:with-fixture successful-jug-pouring-problem ()
      (let ((action (operator domain '!empty))
        ...
        (fiveam:is (equalp '(PDDL-ACTION (!EMPTY ?JUG1 ?JUG2)
                             (AND (%ENFORCE-TYPE-CONSTRAINTS (JUG ?JUG1) (JUG ?JUG2))
